...
namespace Peak.WordFresh.Scripts.Editor
{
    public static class PeakPostProcessBuild
    {
        ...
        private static void RunPostProcessTasksIos(string projectPath)
        {
            ...
            PlistElementString bundleString = new PlistElementString($"{Application.identifier}");
            root[bundleIDKey] = bundleString;
            Debug.Log($"[PostProcessBuild] Bundle id :{bundleString.AsString()} for {Application.identifier}");

            Debug.Log("[PostProcessBuild] Prepare ad network Info.plist update.");

            const string GADAppID = "GADApplicationIdentifier";
            PlistElementString AdMobAppID = new PlistElementString("ca-app-pub-1582776119070207~9838890482");
            root[GADAppID] = AdMobAppID;
