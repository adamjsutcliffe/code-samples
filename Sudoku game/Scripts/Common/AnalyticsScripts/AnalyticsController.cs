...
namespace Peak.Speedoku.Scripts.Common.AnalyticsScripts
{
    ...
    public sealed class AnalyticsController : MonoBehaviour
    {
        ...
        private string lastFtueStep = "0_None";
        ...
        {
            AnalyticsResult analyticsResult = AnalyticsEvent.TutorialStart("FTUE");
            DebugLog($"[ANALYTICS][FTUE] [{analyticsResult}] Tutorial started");
        }

        public void TutorialStepComplete(int index, string ftueStep)
        {
            lastFtueStep = $"{index}_{ftueStep}";
            AnalyticsResult analyticsResult = AnalyticsEvent.TutorialStep(index, ftueStep);
            DebugLog($"[ANALYTICS][FTUE] [{analyticsResult}] Tutorial STEP ({index}) {ftueStep} COMPLETE");
        }

        public void TutorialStepSkipped()
        {
            AnalyticsResult analyticsResult = AnalyticsEvent.TutorialSkip(lastFtueStep);
            DebugLog($"[ANALYTICS][FTUE] [{analyticsResult}] Tutorial skipped");
        }
        ...
        public void TutorialFinished()
        {
            ...
            Adjust.trackEvent(adjustEvent);
            AnalyticsResult analyticsResult = AnalyticsEvent.TutorialComplete();
            DebugLog($"[ANALYTICS][FTUE] [{analyticsResult}] Tutorial COMPLETE");
        }
